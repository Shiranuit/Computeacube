    ...
    implements ITerminalTile
{
    ...
    public void monitorTouched( float xPos, float yPos, float zPos )
    {
        ...
        int lxCharPos = (int)Math.min((double)originTerminal.getWidth(), Math.max(((pair.x - RENDER_BORDER - RENDER_MARGIN) / (xCharWidth/2)) + 1.0, 1.0));
        int lyCharPos = (int)Math.min((double)originTerminal.getHeight(), Math.max(((pair.y - RENDER_BORDER - RENDER_MARGIN) / (yCharHeight/3)) + 1.0, 1.0));
        double dxCharPos = Math.min((double)originTerminal.getWidth(), Math.max(((pair.x - RENDER_BORDER - RENDER_MARGIN) / xCharWidth) + 1.0, 1.0));
        double dyCharPos = Math.min((double)originTerminal.getHeight(), Math.max(((pair.y - RENDER_BORDER - RENDER_MARGIN) / yCharHeight) + 1.0, 1.0));
        ...
        for( int y=0; y<m_height; ++y )
        {
            for( int x=0; x<m_width; ++x )
            {
                ...
                if( monitor != null )
                {
                    ...
                    monitor.queueLTouchEvent(lxCharPos, lyCharPos);
                    monitor.queueDTouchEvent(dxCharPos, dyCharPos);
    ...
    private void queueLTouchEvent( int xCharPos, int yCharPos )
    {
        for( IComputerAccess computer : m_computers )
        {
            computer.queueEvent( "lmonitor_touch", new Object[] {
                computer.getAttachmentName(), xCharPos, yCharPos
            } );
        }
    }
    ...
    private void queueDTouchEvent( double xCharPos, double yCharPos )
    {
        for( IComputerAccess computer : m_computers )
        {
            computer.queueEvent( "dmonitor_touch", new Object[] {
                computer.getAttachmentName(), xCharPos, yCharPos
            } );
        }
    }
