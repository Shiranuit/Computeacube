...
public class TurtleSuckCommand implements ITurtleCommand
{
    ...
    public TurtleSuckCommand( InteractDirection direction, int quantity, int slot )
    {
        m_direction = direction;
        m_quantity = quantity;
        m_slot = slot;
    ...
    public TurtleCommandResult execute( @Nonnull ITurtleAccess turtle )
    {
        ...
        if( inventory != null )
        {
        	...
        	if (m_slot < 1 || m_slot > inventory.getSlots()) {
        		stack = InventoryUtil.takeItems( m_quantity, inventory );
        	} else {
        		stack = InventoryUtil.takeItems( m_quantity, inventory, m_slot-1, 1, m_slot-1);
        	}
