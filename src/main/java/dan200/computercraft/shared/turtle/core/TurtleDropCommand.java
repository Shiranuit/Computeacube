...
public class TurtleDropCommand implements ITurtleCommand
{
    ...
    public TurtleDropCommand( InteractDirection direction, int quantity, int slot )
    {
        m_direction = direction;
        m_quantity = quantity;
        m_slot = slot;
    ...
    public TurtleCommandResult execute( @Nonnull ITurtleAccess turtle )
    {
        ...
        if( inventory != null )
        {
        	...
        	if (m_slot < 1 || m_slot > inventory.getSlots()) {
        		remainder = InventoryUtil.storeItems( stack, inventory );
        	} else {
        		remainder = InventoryUtil.storeItems( stack, inventory, m_slot-1, 1, m_slot-1 );
        	}
