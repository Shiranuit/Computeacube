...
public class TurtleCraftCommand implements ITurtleCommand
{
    ...
    private int m_craftingtable = -1;
    public TurtleCraftCommand( int limit )
    {
        m_limit = limit;
    }
    
    public TurtleCraftCommand( int limit, int craftingtable )
    {
        m_limit = limit;
    ...
    public TurtleCommandResult execute( @Nonnull ITurtleAccess turtle )
    {
        ...
        ArrayList<ItemStack> results = crafting.doCrafting( turtle.getWorld(), m_limit, m_craftingtable );
        if( results != null )
        {
            // Store the results
