...
public class LuaJLuaMachine implements ILuaMachine
{
    ...
    @Override
    public void hideAPI(String name, String substitute) {
    	LuaValue _table = m_globals.get("table");
    	LuaValue _meta = _table.getmetatable();
    	if (_meta == null) {
    		_meta = new LuaTable();
    		_table.setmetatable(_meta);
    		_meta.set("__unm", new OneArgFunction() {
				@Override
				public LuaValue call(LuaValue tab) {
					return new OneArgFunction() {
						@Override
						public LuaValue call(LuaValue sub) {
							return m_hiddenAPIs.get(sub);
						}
					};
				}
    		});
