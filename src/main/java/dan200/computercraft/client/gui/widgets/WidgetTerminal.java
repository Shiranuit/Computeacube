            ...
            if( m_focus )
            {
                ...
                if( !m_locked && computer != null && computer.isColour() && button >= 0 && button <= 2 )
                {
                    ...
                    if( term != null )
                    {
                        ...
                        int lcharX = ( mouseX - (getXPosition() + m_leftMargin)) / (FixedWidthFontRenderer.FONT_WIDTH/2);
                        int lcharY = ( mouseY - (getYPosition() + m_topMargin)) / (FixedWidthFontRenderer.FONT_HEIGHT/3);
                        charX = Math.min( Math.max( charX, 0 ), term.getWidth() - 1 );
                        charY = Math.min( Math.max( charY, 0 ), term.getHeight() - 1 );
                        lcharX = Math.min( Math.max( lcharX, 0 ), term.getWidth()*2 - 1 );
                        lcharY = Math.min( Math.max( lcharY, 0 ), term.getHeight()*3 - 1 );

                        computer.queueEvent( "mouse_click", new Object[]{
                            button + 1, charX + 1, charY + 1
                        } );
                        computer.queueEvent( "lmouse_click", new Object[]{
                                button + 1, lcharX + 1, lcharY + 1
                            } );
                        computer.queueEvent( "dmouse_click", new Object[]{
                                button + 1, mouseX + 1, mouseY + 1
                            } );
            ...
            if( term != null )
            {
                ...
                if( m_lastClickButton >= 0 && !Mouse.isButtonDown( m_lastClickButton ) )
                {
                    if( m_focus && !m_locked )
                    {
                        ...
                        computer.queueEvent( "lmouse_up", new Object[]{
                                m_lastClickButton + 1, lcharX + 1, lcharY + 1
                            } );
                        computer.queueEvent( "dmouse_up", new Object[]{
                                m_lastClickButton + 1, mouseX + 1, mouseY + 1
                            } );
                ...
                if( m_focus && !m_locked )
                {
                    if( wheelChange < 0 )
                    {
                        ...
                        computer.queueEvent( "lmouse_scroll", new Object[]{
                                1, lcharX + 1, lcharY + 1
                        } );
                        computer.queueEvent( "dmouse_scroll", new Object[]{
                                1, mouseX + 1, mouseY + 1
                        } );
                    }
                    ...
                    if( m_lastClickButton >= 0 && ( charX != m_lastClickX || charY != m_lastClickY ) )
                    {
                        ...
                        computer.queueEvent( "lmouse_drag", new Object[]{
                                m_lastClickButton + 1, lcharX + 1, lcharY + 1
                            } );
                        computer.queueEvent( "dmouse_drag", new Object[]{
                                m_lastClickButton + 1, mouseX + 1, mouseY + 1
                            } );
                        m_lastClickX = charX;
                        m_lastClickY = charY;
